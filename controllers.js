...
export const addMovie = ((req, res) => {
    ...
    if(Object.getOwnPropertyNames(movie).length === 3 && propertiesIsOk === true){
        const createUuid = uuidv4();
        movie = {id: createUuid, ...movie};
        movies.push(movie);

        res.status(200).send(`${movie.title} movie with id (${createUuid}) has been added to the list`);
    } else {
        res.status(400).send("You just need to send title, director and release date!");
    }
})
...
export const removeMovie = ((req, res) => {
    ...
    if (isIdExist) {
        ...
        res.status(200).send(`The movie with id (${id}) has been deleted from the list`);
    } else {
